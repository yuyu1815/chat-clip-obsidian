...
const OptionsApp = () => {
    ...
    chrome.storage.sync.get(
      ...
      (result) => {
        ...
        if (result.folderPath) {
          console.log('[ChatVault Options] 📁 Setting folder:', result.folderPath);
          setFolder(result.folderPath);
        } else {
          console.log('[ChatVault Options] ⚠️ No folder found in storage');
        }
  ...
  const handleTest = () => {
    ...
    const title = showChatSettings ? "ChatVault Test Note" : "Obsidian Web Clipper Test Note";
    const url = showChatSettings ? "https://chat.openai.com/test" : "https://example.com";
    const content = showChatSettings 
      ? "### User\n\nTest question\n\n### Assistant\n\nThis is a test response from ChatVault Clip!"
      : "This is a test note generated by the Obsidian Web Clipper extension.";
    const date = new Date().toISOString().split("T")[0];
    const service = "ChatGPT";

    let formattedContent;
    const format = showChatSettings ? chatNoteFormat : (showAdvancedFeatures ? noteContentFormat : `${url}\n\n${content}`);
    ...
    const testFolder = showChatSettings ? chatFolderPath : folder;
    
    let folderPath = testFolder.trim()
      .replace("{title}", title)
      .replace("{service}", service)
      .replace("{date}", date);
    ...
    <div className="bg-zinc-800 px-32 py-8 min-h-screen">
      ...
      <div className="my-8">
        <label className="text-white text-lg">
          Web Clips Folder{" "}
          <span className="text-gray-500 text-base">
            ( Folder path for traditional web clippings. Use {"{title}"} as placeholder )
          </span>
        </label>
        <input
          required
      ...
      <div className="mt-8 bg-red-950 p-4 rounded-md flex items-center justify-between text-white">
        <label className="text-lg">
          ...
          />
          Enable Advanced Web Clip Formatting
        </label>
      </div>
